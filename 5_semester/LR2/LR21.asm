; Почему именно 1,2... сдвинутые на 3
...
; откуда берутся эти числа, Почему именно они? 
...
start:
    ...
    ; Очистка экрана через видеопамять, тут просьба пояснить почему именно такие числа тут.
    mov     cx, 80 * 25 * 2
    mov     ax, 0xb800
    mov     es, ax
    mov     di, 0
...
main:
    ...
    mov     edi, GDTEND + 8; почему именно +8?
    mov     ebx, TSS_2_BASE
    call    make_tdesc
